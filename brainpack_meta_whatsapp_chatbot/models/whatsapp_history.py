...
class WhatsappHistory(models.Model):
    ...
    channel_id = fields.Many2one(comodel_name='mail.channel', string="Mail Channel")
    ...
    def create(self, vals):
        ...
        message_id = self.env.context.get('wa_message_id', False)
        ...
        if provider_id and partner_id and partner_id.mobile:
            ...
            if not channel:
                ...
                channel = self.env['mail.channel'].sudo().create({
                    ...
                    'channel_partner_ids': [(4, int(vals.get('partner_id')) if vals.get('partner_id') else False),
                                            (4, user_partner.id)],
                })
            ...
            if wa_template and wa_template.template_type == 'interactive' and provider_id and partner_id and partner_id.mobile:
                if not self.env.context.get('whatsapp_application'):
                    if 'template_send' in self.env.context and self.env.context.get('template_send'):
                        ...
                        if wa_template.template_type == 'interactive':
                            answer = provider_id.send_mpm_template(wa_template.name, wa_template.lang.iso_code,
                                                                   wa_template.namespace, partner_id,
                                                                   params)
                ...
                if provider_id and partner_id and partner_id.mobile:
                    ...
                    if vals.get('type') == 'received':
                        message_script = self.env['whatsapp.chatbot'].search([]).mapped('step_type_ids').filtered(
                            lambda l: l.message == vals.get('message'))
                        ...
                        if channel:
                            ...
                            if chatbot_script_lines:
                                for chat in chatbot_script_lines:
                                    message_values.update({'wa_chatbot_id': chat.whatsapp_chatbot_id.id})
                        ...
                        if not self.env.context.get('whatsapp_application'):
                            if 'template_send' in self.env.context and self.env.context.get('template_send'):
                                ...
                                if 'active_model_id_chat_bot' in self.env.context:
                                    active_model_id = self.env.context.get('active_model_id_chat_bot')

                                for component in wa_template.components_ids:
                                    object_data = self.env[wa_template.model_id.model].search_read(
                                        [('id', '=', active_model_id)])[0]
                                ...
                                answer = provider_id.send_template(wa_template.name, wa_template.lang.iso_code,
                                                                   wa_template.namespace, partner_id, params)
                                if answer.status_code == 200:
                                    dict = json.loads(answer.text)
                                    if provider_id.provider == 'graph_api':  # if condition for Graph API
                                        if 'messages' in dict and dict.get('messages') and dict.get('messages')[0].get(
                                        ...
                                        if provider_id.provider == 'graph_api':  # if condition for Graph API
                                            ...
                                            if 'sent' in dict and dict.get('sent'):
                                                ...
                                                if not self.env.context.get('cron'):
                                                    ...
                                                    vals.update({'type': 'fail'})
                                                    if 'message' in dict:
                                                        vals.update({'fail_reason': dict.get('message')})

                                if vals.get('attachment_ids'):
                                    for attachment in vals.get('attachment_ids'):
                                        if attachment[1]:
                                            attachment_id = self.env['ir.attachment'].sudo().browse(attachment[1])

                                            if provider_id.provider == 'chat_api':
                                                answer = provider_id.send_file(partner_id, attachment_id)
                                                if answer.status_code == 200:
                                                    dict = json.loads(answer.text)
                                                    if 'sent' in dict and dict.get('sent'):
                                                        vals['message_id'] = dict['id']
                                                        if self.env.context.get('wa_messsage_id'):
                                                            self.env.context.get('wa_messsage_id').wa_message_id = dict[
                                                                'id']
                                                    else:
                                                        if not self.env.context.get('cron'):
                                                            if 'message' in dict:
                                            ...
                                            if provider_id.provider == 'graph_api':
                                                ...
                                                answer = provider_id.send_image(partner_id, attachment_id)
                                                if answer.status_code == 200:
                                                    dict = json.loads(answer.text)
                                                    media_id = dict.get('id')
                                                    getimagebyid = provider_id.get_image_by_id(media_id, partner_id,
                                                    ...
                                                    if 'messages' in imagedict and imagedict.get('messages'):
                                                        vals['message_id'] = imagedict.get('id')
                                                        if self.env.context.get('wa_messsage_id'):
                                                        ...
                                                        if not self.env.context.get('cron'):
                                                            ...
                                                            vals.update({'type': 'fail'})
                                                            if 'messages' in imagedict:
                                                                vals.update({'fail_reason': imagedict.get('message')})
                ...
                if vals.get('message') and vals.get('type') == 'received':
                    ...
                    if provider_id.company_id and provider_id.company_id.wa_chatbot_id:
                        # Bot
                        if provider_id.company_id.wa_chatbot_id.mapped('step_type_ids').filtered(
                                lambda l: l.message == vals.get('message')):
                            channel.sudo().write({
                                'wa_chatbot_id': provider_id.company_id.wa_chatbot_id.id if provider_id.company_id and provider_id.company_id.wa_chatbot_id else False,
                            })

                        if channel.wa_chatbot_id and provider_id.company_id.wa_chatbot_id == channel.wa_chatbot_id and not channel.is_chatbot_ended:
                            message_script = self.env['whatsapp.chatbot'].search(
                                [('id', '=', channel.wa_chatbot_id.id)]).mapped('step_type_ids').filtered(
                                lambda l: l.message == vals.get('message'))
                            current__chat_seq_script = self.env['whatsapp.chatbot'].search(
                                [('id', '=', channel.wa_chatbot_id.id)]).mapped(
                                'step_type_ids').filtered(
                                lambda l: l.sequence == channel.script_sequence)

                            most_parent = message_script
                            chatbot_script_lines = message_script

                            if not chatbot_script_lines and channel.wa_chatbot_id.step_type_ids:
                                chatbot_script_lines = channel.wa_chatbot_id.step_type_ids[0]
                            ...
                            for chat in chatbot_script_lines:
                                if chat.sequence >= channel.script_sequence:
                                ...
                                elif current__chat_seq_script and current__chat_seq_script.parent_id and current__chat_seq_script.parent_id == chat.parent_id:
                                    for chat in chatbot_script_lines:
                                        channel.write({
                                            'wa_chatbot_id': chat.whatsapp_chatbot_id.id,
                                            'script_sequence': chat.sequence,
                                    ...
                                    first_script = self.env['whatsapp.chatbot'].search(
                                        [('id', '=', channel.wa_chatbot_id.id)]).mapped(
                                        'step_type_ids').filtered(
                                        lambda l: l.sequence == 1)
                                ...
                                if chat.step_call_type == 'message':
                                    ...
                                    if not self.env.context.get('whatsapp_application'):
                                        ...
                                        message = self.env['mail.message'].sudo().with_context(
                                            {'provider_id': provider_id}).create(
                                            message_values)
                                        ...
                                        #     parent_msg = self.env['mail.message'].sudo().search(
                                        #         [('id', '=', self.env.context.get('message_parent_id').id)])
                                        #     answer = provider_id.send_message(partner_id, chat_answer,
                                        #                                       parent_msg.wa_message_id)
                                        # else:
                                        #     answer = provider_id.send_message(partner_id, chat_answer)
                                        # if answer.status_code == 200:
                                        ...
                                        #                 if 'error' in dict:
                                        #                     vals.update(
                                        #                         {'fail_reason': dict.get('error').get('message')})
                                        ...
                                        if template:
                                            ...
                                            if template.body_html != '':
                                                wa_message_values.update(
                                                    {'body': tools.html2plaintext(template.body_html)})
                                            ...
                                            wa_attach_message = self.env['mail.message'].sudo().with_user(
                                                provider_id.user_id.id).with_context(
                                                {'template_send': True, 'wa_template': template,
                                                 'active_model_id': channel.id, 'active_model': 'mail.channel',
                                                 'active_model_id_chat_bot': partner_id.id,
                                                 'active_model_chat_bot': 'res.partner',
                                                 'provider_id': provider_id}).create(
                                                wa_message_values)  # 'attachment_ids': template.attachment_ids.ids,
                                ...
                                if chat.step_call_type == 'interactive':
                                    ...
                                    if not self.env.context.get('whatsapp_application'):
                                        ...
                                        if template:
                                            wa_message_body = self.env['mail.message'].with_user(
                                                provider_id.user_id.id).with_context(
                                                {'template_send': True, 'wa_template': template,
                                                 'provider_id': provider_id, 'user_id': provider_id.user_id}).create(
                                                message_values)
                                        else:
                                ...
                                if chat.step_call_type == 'action':
                                    ...
                                    if chat.action_id:
                                        if chat.action_id.binding_model_id.model == 'crm.lead':
                                            ...
                                            message = self.env['mail.message'].sudo().with_user(
                                                provider_id.user_id.id).with_context(
                                                {'provider_id': provider_id}).create(
                                                message_values)
                                            notifications = channel._channel_message_notifications(message)
                                            ...
                                            lead = self.env['crm.lead'].with_user(
                                                provider_id.user_id.id).create({
                                            ...
                                            #     parent_msg = self.env['mail.message'].sudo().search(
                                            #         [('id', '=', self.env.context.get('message_parent_id').id)])
                                            #     answer = provider_id.send_message(partner_id, lead_message,
                                            #                                       parent_msg.wa_message_id)
                                            # else:
                                            #     answer = provider_id.send_message(partner_id, lead_message)
                                            # if answer.status_code == 200:
                                            ...
                                            if lead:
                                                if channel:
                                                    channel.write({'is_chatbot_ended': True, 'wa_chatbot_id': False})
                                                    ...
                                                    if active_operator:
                                                        ...
                                                        for wa_channel in wa_chatbot_channels:
                                                            operators = active_operator.filtered(lambda
                                                                                                     av_user: av_user.partner_id not in wa_channel.channel_member_ids.partner_id)
                                                            if operators:
                                                                for operator in operators:
                                                                    available_operator = operator.partner_id
                                                            else:
                                                                available_operator = active_operator[0].partner_id
                                                        ...
                                                        if available_operator:
                                                            ...
                                                            mail_channel_partner = self.env[
                                                                ...
                                                                [('channel_id', '=', channel.id),
                                                                 ('partner_id', '=', available_operator.id)])
                                                            mail_channel_partner.write({'is_pinned': True})
                                                    else:
                                                        channel.write({'is_chatbot_ended': False,
                                                                       'wa_chatbot_id': chat.whatsapp_chatbot_id.id})

                                        if chat.action_id.binding_model_id.model == 'mail.channel':
                                            available_operator = False
                                            ...
                                            if active_operator:
                                                ...
                                                if available_operator:
                                                    if channel.whatsapp_channel:
                                                        channel.write(
                                                            {'channel_partner_ids': [(4, available_operator.id)],
                                                             'is_chatbot_ended': True, 'wa_chatbot_id': False})
                                                        mail_channel_partner = self.env[
                                                            'mail.channel.member'].sudo().search(
                                                            [('channel_id', '=', channel.id),
                                                             ('partner_id', '=', available_operator.id)])
                                                    ...
                                                    # answer = provider_id.send_message(partner_id, user_message)
                                                    # if answer.status_code == 200:
                                                    #     dict = json.loads(answer.text)
                                                    #     if provider_id.provider == 'graph_api':  # if condition for Graph API
                                                    #         if 'messages' in dict and dict.get('messages') and \
                                                    #                 dict.get('messages')[
                                                    #                     0].get(
                                                    #                     'id'):
                                                    #             message_id = dict.get('messages')[0].get('id')
                                                    ...
                                                    #                 if 'error' in dict:
                                                    #                     vals.update(
                                                    #                         {'fail_reason': dict.get('error').get(
                                                    #                             'message')})

                                                    user_message = "Yor are now chatting with " + available_operator.name
                                                    ...
                                                    #             if self.env.context.get('wa_messsage_id'):
                                                    #                 self.env.context.get(
                                                    #                     'wa_messsage_id').wa_message_id = dict[
                                                    #                     'id']
                                                    #         else:
                                                    #             if not self.env.context.get('cron'):
                                                    #                 if 'message' in dict:
                                                ...
                                                user_message = "We will getting you soon."
                                                message_values = {
                                                    'body':  user_message,
                                                    'author_id': user_partner.id,
                                                    'email_from': user_partner.email or '',
