...
class CalendarEvent(models.Model):
    ...
    def _track_template(self, changes):
        ...
        if 'appointment_type_id' in changes:
            booked_template = self.env.ref('brainpack_appointment.appointment_booked_mail_template').sudo()
            res['appointment_type_id'] = (booked_template, {
                **author,
                'auto_delete_message': True,
                'subtype_id': self.env['ir.model.data']._xmlid_to_res_id('brainpack_appointment.mt_calendar_event_booked'),
                'email_layout_xmlid': 'mail.mail_notification_light'
            })
        if 'active' in changes and not self.active and self.start > fields.Datetime.now():
            canceled_template = self.env.ref('brainpack_appointment.appointment_canceled_mail_template').sudo()
            res['active'] = (canceled_template, {
                **author,
                'auto_delete_message': True,
                'subtype_id': self.env['ir.model.data']._xmlid_to_res_id('brainpack_appointment.mt_calendar_event_canceled'),
                'email_layout_xmlid': 'mail.mail_notification_light'
            })
        return res
