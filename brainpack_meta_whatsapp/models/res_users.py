...
class ResUsers(models.Model):
    ...
    #         )
    #     if self.provider_id.id not in all_provider_by_company.ids:
    #         raise ValidationError(
    ...
    def write(self, values):
        ...
        if values.get('groups_id'):
            ...
            uncheck_ids = []
            whatsapp_manager_group = self.env.ref('brainpack_meta_whatsapp.whatsapp_group_manager').id
            inherited_groups = self.env['res.groups'].search(
                [('implied_ids', 'in', self.env.ref('brainpack_meta_whatsapp.whatsapp_group_user').ids)])
            for group in groups:
