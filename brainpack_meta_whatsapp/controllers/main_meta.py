...
class WebHook2(http.Controller):
    _webhook_url = '/graph_brainpack/webhook'
    _meta_fb_url = '/graph_brainpack/webhook'
                            ...
                            for mes in data.get('entry')[0].get('changes')[0].get('value').get('messages'):
                                ...
                                for partner in partners:
                                    ...
                                    if mes.get('type') == 'text':
                                        ...
                                        title = list(map(lambda l: mes.get('interactive').get(l), mes.get('interactive')))
                                        vals = {
                                            'provider_id': provider.id,
                                            'author_id': user_partner.id,
    ...
    #     # if wizard open
    #     # context = {'active_model':active_model,'active_id':int(kw.get('partner_id')),'default_partner_id':int(kw.get('partner_id')),'default_provider_id':int(kw.get('template_id'))}
    #     # action = request.env.ref('brainpack_meta_whatsapp.action_wa_compose_message_wizard').read()[0]
